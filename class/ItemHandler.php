 ...
 * @version		$Id: ItemHandler.php 378 2013-09-17 11:08:54Z qm-b $
 * @package		menu
 *
 */

defined("ICMS_ROOT_PATH") or die("ICMS root path not defined");
if(!defined(MENU_DIRNAME)) define(MENU_DIRNAME, basename(dirname(__FILE__, 2)));
...
class mod_menu_ItemHandler extends icms_ipf_Handler {
    ...
    public function getItemCriterias($act = FALSE, $item_pid = NULL, $menu_id = FALSE, $start=0,$limit=0,$perm="item_view", $order ="weight" , $sort ="DESC", $lang = FALSE ) {
        global $MENU_ITEM_ORDER, $MENU_ITEM_SORT, $icmsConfigMultilang;
        ...
        if($icmsConfigMultilang['ml_enable']) {
            ...
            $tray->add(new icms_db_criteria_Item("language", icms::$config->getConfig('language'), 'OR'));
            $criteria->add($tray);
        }
        return $criteria;
    }
    ...
    public function getItemListForPid($act=FALSE, $item_id=NULL, $menu_id = MENU_MENU_ID, $start=0,$limit=0,$perm="item_view",$showNull = TRUE) {
        $menu_handler = icms_getModuleHandler("menu", MENU_DIRNAME, "menu");
        $criteria = $this->getItemCriterias($act, $item_id, $menu_id, $start, $limit, $perm);
        ...
        foreach(array_keys($items) as $i) {
            ...
            $subitems = $this->getItemListForPid($act, $items[$i]->id(), $menu_id, $start, $limit, $perm, FALSE);
            if($subitems) {
